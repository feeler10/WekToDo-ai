...
任务删除与 `CANCELLED` 状态更新是两个独立操作。取消任务保留任务对象并改变状态；删除任务在用户确认后物理移除任务对象，且不可恢复。删除意图必须先从 Redis 唯一定位真实任务，多候选时请求用户选择，确认时重新校验 `user_id`、任务版本和当前父子关系。

第一阶段只允许删除叶子任务，不执行隐式级联删除。存在直接子任务的任务必须拒绝删除；子任务仍被同一父任务下其他子任务依赖时也必须拒绝删除，避免产生悬空依赖。删除子任务时，任务对象、用户任务索引、父任务子节点有序索引、父任务进度/完成状态/完成时间/版本以及删除幂等结果必须在同一个 Redis 事务中全有或全无地更新。

删除后按剩余有效子任务重新计算父任务进度，`CANCELLED` 不计入有效数量。剩余有效子任务全部完成时父任务保持或更新为 `DONE`；剩余有效子任务不全完成，或有效数量变为零时，原本因子任务闭环完成的父任务恢复为 `DOING`、清空 `completed_at`，有效数量为零时 `progress=0`。该父任务联动属于已确认删除的确定性副作用，不重复请求确认。

### 8.8.2 自然语言批量删除与已取消任务恢复

删除意图使用独立结构化 LLM 解析器。模型仅把自然语言转换为时间范围、状态、优先级、分类、任务引用、关键词和“全部匹配”等受控参数，不接触 Redis、不决定真实任务 ID、也不执行删除。任意明确自然语言日期范围转换为带时区的左闭右开边界；关键词范围在标题、描述和分类中执行确定性包含匹配。后端根据参数查询 Redis，完整展示最终匹配任务，获得确认后才执行删除。

批量删除单批最多 50 条，所有目标必须重新通过归属、版本、叶子约束和依赖约束校验。任务对象、用户索引、父任务子节点索引、所有受影响父任务的汇总和输入绑定的幂等记录必须在同一个 `WATCH/MULTI/EXEC` 事务中全有或全无地提交，禁止部分成功。

`CANCELLED` 仍表示任务已取消。用户查询单个已取消任务时应提示恢复规则；用户对其发起属性修改、状态更新或任务拆解时，系统必须先产生独立恢复确认。只有确认执行 `CANCELLED → TODO` 后，Graph 才自动继续原始意图，并为后续写操作再次请求确认。拒绝恢复时保持原状态且不执行原操作。删除是独立意图，可以直接删除已取消任务，无需先恢复。

删除范围必须区分全用户任务与指定父任务的直接子任务。`DIRECT_CHILDREN` 范围必须携带 `parent_reference`，父任务唯一定位后才能在其直接子任务集合中应用名称、状态、时间、分类或关键词过滤。不得把其他父任务下的同名子任务混入删除计划。

多个显式任务引用必须分别产生解析结果。只有全部引用都唯一匹配时才能生成确认计划；任一引用未找到时必须停止整个计划并明确报告缺失项，禁止部分删除；任一引用存在多个候选时必须保存当前删除参数、已解析目标和候选版本，通过跨轮选择逐项消歧。Graph 或服务重建后，该选择状态必须能够从 Checkpoint 恢复。
